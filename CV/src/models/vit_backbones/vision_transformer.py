...
class Attention(nn.Module):
    ...
    def forward(self, x, prefix_key=None, prefix_value=None, prompt=None):
        ...
        if prompt is not None:
            ...
            prompt_qkv = self.qkv(prompt).reshape(B, prompt_len, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
            _, prefix_key, prefix_value = prompt_qkv.unbind(0)   # make torchscript happy (cannot use tensor as tuple)

            k = torch.cat([prefix_key, k], dim=1)
            v = torch.cat([prefix_value, v], dim=1)
        elif prefix_key is not None:
            prompt_len = prefix_key.shape[1]

            prefix_key = prefix_key.reshape(B, prompt_len, self.num_heads, -1).permute(0, 2, 1, 3)
            prefix_value = prefix_value.reshape(B, prompt_len, self.num_heads, -1).permute(0, 2, 1, 3)
...
class Block(nn.Module):
    ...
    def forward(self, x,  prefix_key=None, prefix_value=None, prompt=None):
        if prompt is not None:
            prompt = self.norm1(prompt)

        x = x + self.drop_path(self.attn(self.norm1(x), prefix_key, prefix_value, prompt))
