...
class Attention(nn.Module):
    ...
    def forward(self, hidden_states, prefix_key=None, prefix_value=None, prompt=None):
        ...
        B, num_head, N, C = value_layer.shape

        if prompt is not None:
            prompt_length = prompt.shape[1]

            prefix_key = self.key(prompt)
            prefix_value = self.value(prompt)

            prefix_key = prefix_key.reshape(B, prompt_length, num_head, -1).permute(0, 2, 1, 3) # B, num_head, L, head_size
            prefix_value = prefix_value.reshape(B, prompt_length, num_head, -1).permute(0, 2, 1, 3) # B, num_head, L, head_size
            ...
            prompt_length = prefix_key.shape[1]
...
class Block(nn.Module):
    ...
    def forward(self, x, prefix_key=None, prefix_value=None, prompt=None):
        h = x
        x = self.attention_norm(x)
        if prompt is not None:
            prompt = self.attention_norm(prompt)

        x, weights = self.attn(x, prefix_key, prefix_value, prompt)
        x = x + h
