...
class PromptedTransformer(Transformer):
    ...
    def forward_deep_prompt(self, embedding_output, kv_only=False):
        attn_weights = []
        hidden_states = None
        weights = None
        ...
        for i in range(num_layers):
            if kv_only:
                if i == 0:
                    ...
                    hidden_states, weights = self.encoder.layer[i](embedding_output, prompt=prompt)
                elif i < self.prompt_depth and self.prompt_config.DEEP:
                    prompt = self.prompt_dropout(self.prompt_proj(self.deep_prompt_embeddings[i-1]).expand(B, -1, -1))
                    hidden_states, weights = self.encoder.layer[i](hidden_states, prompt=prompt)
                else:
    ...
    def forward(self, x):
        if self.prompt_config.KV_ONLY:
            ...
            embedding_output = self.embeddings(x)  # (batch_size, 1 + n_patches, hidden_dim)

            encoded, attn_weights = self.forward_deep_prompt(
                embedding_output, kv_only=True
            )
...
class PrefixTransformer(Transformer):
    def __init__(self, prompt_config, config, img_size, vis):
        ...
        num_tokens = self.prompt_config.NUM_TOKENS
        self.num_tokens = num_tokens  # number of prompted tokens
        self.num_experts = num_tokens // 2  # number of experts encoded in the prompt
        ...
        if self.prompt_config.INITIATION == "random":
            ...
            if not self.prompt_config.DEEP:
                total_layer = 1

            self.prompt_depth = total_layer

            if prompt_config.SHARE_KV:
                self.prompt_embeddings = nn.Parameter(torch.zeros(
                    total_layer, self.num_experts, prompt_dim))
                ...
                if prompt_config.SHARE_TYPE == "non-linear":
                    self.prompt_trans = nn.ModuleList([
                        nn.Sequential(
                            nn.Linear(prompt_dim, 48),
                            nn.Tanh(),
                            nn.Linear(48, 2 * prompt_dim)
    ...
    def forward_deep_prompt(self, embedding_output, deep=True):
        attn_weights = []
        ...
        for i in range(num_layers):
            if i == 0 or (i < self.prompt_depth and deep):
                if self.prompt_config.SHARE_KV:
                    prefix_key_value = self.prompt_trans[i](
                        self.prompt_embeddings[i]
                    )  # (num_experts, 2 * prompt_dim)

                    prefix_key_value = prefix_key_value.reshape(
                        self.num_experts, -1, 2
                    ).permute(2, 0, 1)  # (2, num_experts, prompt_dim)

                    prefix_key = self.prompt_dropout(self.prompt_proj(prefix_key_value[0]).expand(B, -1, -1))
                    prefix_value = self.prompt_dropout(self.prompt_proj(prefix_key_value[1]).expand(B, -1, -1))
                else:
                    prefix_key = self.prompt_dropout(self.prompt_proj(self.prompt_key_embeddings[i]).expand(B, -1, -1))
                    prefix_value = self.prompt_dropout(self.prompt_proj(self.prompt_value_embeddings[i]).expand(B, -1, -1))

                hidden_states, weights = self.encoder.layer[i](hidden_states, prefix_key, prefix_value)
    ...
    def forward(self, x):
        ...
        encoded, attn_weights = self.forward_deep_prompt(
            embedding_output, deep=self.prompt_config.DEEP
        )
...
class PrefixVisionTransformer(VisionTransformer):
    def __init__(
        self, prompt_cfg, model_type,
        img_size=224, num_classes=21843, vis=False
    ):
        assert prompt_cfg.VIT_POOL_TYPE == "original"
        super(PrefixVisionTransformer, self).__init__(
            model_type, img_size, num_classes, vis)
        if prompt_cfg is None:
            raise ValueError("prompt_cfg cannot be None if using PromptedVisionTransformer")
        self.prompt_cfg = prompt_cfg
        vit_cfg = CONFIGS[model_type]
        self.transformer = PrefixTransformer(
            prompt_cfg, vit_cfg, img_size, vis)
